...
export default class AnchorPostDisplay extends Component {
    ...
    render() {
        ...
        return (
            //【jc change】：间隔改为不透明的view填充，以便下拉刷新时不会漏出后面的字
        ...
        if (IS_5_5_SCREEN) {
            return (
                <View style={styles.anchorPicContainer}>
                    <Image style={styles.anchorPicPlaceholder}
                           source={require('./images/LiveLobby/live_list_placeholder_1242.png')}>
                    </Image>
                    <Image style={styles.anchorPic} source={{uri: this.props.dataDic.pospic}}>
                    </Image>
                    <Image style={styles.anchorShadow}
                           source={require('./images/LiveLobby/live_list_image_banner_mask.png')}>
                    </Image>
                </View>
            );
            ...
            return (
                <View style={styles.anchorPicContainer}>
                    <Image style={styles.anchorPicPlaceholder}
                           source={require('./images/LiveLobby/live_list_placeholder_750.png')}>
                    </Image>
                    <Image style={styles.anchorPic} source={{uri: this.props.dataDic.pospic}}>
                    </Image>
                    <Image style={styles.anchorShadow}
            ...
            return (
                <View style={styles.anchorPicContainer}>
                    <Image style={styles.anchorPicPlaceholder}
                           source={require('./images/LiveLobby/live_list_placeholder_640.png')}>
                    </Image>
                    <Image style={styles.anchorPic} source={{uri: this.props.dataDic.pospic}}>
                    </Image>
                    <Image style={styles.anchorShadow}
    ...
    renderTags() {
        ...
        for (let i = 0; i < this.props.dataDic.tagids.length; i++) {
            ...
            for (let j = 0; j < this.props.arrTags.length; j++) {
                if (tagID == this.props.arrTags[j].id) {
                    if (IS_3X_DEVIDE) {
                        tagsPic.push(this.props.arrTags[j].viewPicSmall.img3x);
                        tagsWidth.push(this.props.arrTags[j].viewPicSmall.img3xw / 3);
                    } else {
                        tagsPic.push(this.props.arrTags[j].viewPicSmall.img2x);
                        tagsWidth.push(this.props.arrTags[j].viewPicSmall.img2xw / 2);
                    }
                    break;
                }
            }
    ...
    renderCorners() {
        return (
            <View style={styles.cornerView}>
                <Image style={styles.topLeftCorner}
                       source={require('./images/LiveLobby/live_list_icon_corner_topLeft.png')}>
                </Image>
                <Image style={styles.topRightCorner}
                       source={require('./images/LiveLobby/live_list_icon_corner_topRight.png')}>
                </Image>
                <Image style={styles.bottomLeftCorner}
                       source={require('./images/LiveLobby/live_list_icon_corner_bottomLeft.png')}>
                </Image>
                <Image style={styles.bottomRightCorner}
                       source={require('./images/LiveLobby/live_list_icon_corner_bottomRight.png')}>
                </Image>
            </View>
        );
    }
...
const styles = StyleSheet.create({
    bgView: {
        ...
        marginLeft: 0,// [JC change] ANCHOR_POST_GAP
        marginTop: 0,
        width: (SCREEN_WIDTH - ANCHOR_POST_GAP * 3) / 2,
        height: (SCREEN_WIDTH - ANCHOR_POST_GAP * 3) / 2 + 36,
